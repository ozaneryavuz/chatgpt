...
assert.equal(run({}).recommendedCurrent,16);
assert.equal(run({ownership:'owned'}).noPurchaseNeeded,true);
assert.equal(run({ownership:'owned'}).commercialAllowed,false);
const single=run({vehicleMaxKw:22,cablePhases:'single',cableRatedCurrent:32});
assert(single.blockerCodes.includes('phase_capacity'));assert.equal(single.commercialAllowed,true);
const low=run({vehiclePhases:'single',stationPhases:'single',vehicleMaxKw:7.4,cablePhases:'single',cableRatedCurrent:16});
assert(low.blockerCodes.includes('current_capacity'));assert.equal(low.commercialAllowed,true);
