(() => {
  ...
  const presets={
    ...
    seven:{stationType:'socketed',vehicleInlet:'type2',vehiclePhases:'single',vehicleMaxKw:7.4,stationPhases:'three',stationMaxCurrent:32,ownership:'candidate',cableConnector:'type2-type2',cablePhases:'single',cableRatedCurrent:32,cableLength:7,labelVerified:true,vehicleSpecVerified:true,stationSpecVerified:true,damageFree:true,storageOk:true,manufacturerCompatibility:true,noExtension:true,lockingWorks:true},
    owned:{stationType:'socketed',vehicleInlet:'type2',vehiclePhases:'three',vehicleMaxKw:11,stationPhases:'three',stationMaxCurrent:32,ownership:'owned',cableConnector:'type2-type2',cablePhases:'three',cableRatedCurrent:32,cableLength:5,labelVerified:true,vehicleSpecVerified:true,stationSpecVerified:true,damageFree:true,storageOk:true,manufacturerCompatibility:true,noExtension:true,lockingWorks:true}
  };
  ...
  function updateAffiliate(){const enabled=$('affiliateAck').checked;const link=$('productCenterLink');link.classList.toggle('disabled-link',!enabled);link.setAttribute('aria-disabled',enabled?'false':'true');link.tabIndex=enabled?0:-1;emit('ev_cable_affiliate_acknowledged',{acknowledged:enabled});}
  function render(result){
    ...
    $('resultStatus').className=`status ${result.status==='compatible'||result.status==='not-needed'?'ok':result.status==='conditional'?'warn':'bad'}`;
    $('targetMetric').textContent=result.targetPowerKw==null?'—':`${fmt(result.targetPowerKw)} kW`;
    $('cableMetric').textContent=result.cablePowerKw==null?'—':`${fmt(result.cablePowerKw)} kW`;
    $('classMetric').textContent=result.recommendedLabel;
    $('limitMetric').textContent=result.limitingComponent;
    ...
    $('warningList').innerHTML=list(result.warnings.length?result.warnings:['Belirgin sınır veya güvenlik sorunu görülmedi; tam model kılavuzları yine doğrulanmalıdır.']);
    ...
    emit('ev_cable_suitability_completed',{status:result.status,commercial_allowed:result.commercialAllowed,no_purchase_needed:result.noPurchaseNeeded,required_class:result.recommendedCurrent?`${result.recommendedPhases}_${result.recommendedCurrent}`:'unknown',ownership:result.input.ownership});
    if(result.noPurchaseNeeded)emit('ev_cable_no_purchase',{reason:result.status==='not-needed'?'tethered_station':'owned_cable_sufficient'});
  }
  ...
  $('productCenterLink').addEventListener('click',event=>{if(event.currentTarget.getAttribute('aria-disabled')==='true'){event.preventDefault();return;}emit('ev_cable_product_center_opened',{category:'ev_sarj_kablosu'});});
  setValues(presets.eleven);
})();