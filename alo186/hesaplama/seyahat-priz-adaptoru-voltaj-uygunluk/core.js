(function(root,factory){
  ...
  const destinations={
    eu:{id:'eu',label:'Türkiye ve Avrupa C/F',plug:'C/F',voltage:230,frequency:50,search:'Avrupa Type C F seyahat adaptörü topraklı'},
    uk:{id:'uk',label:'Birleşik Krallık ve İrlanda G',plug:'G',voltage:230,frequency:50,search:'Türkiye fişi Type G İngiltere seyahat adaptörü topraklı sigortalı'},
    us:{id:'us',label:'ABD ve Kanada A/B',plug:'A/B',voltage:120,frequency:60,search:'Türkiye fişi Type A B ABD seyahat adaptörü topraklı'},
    japan:{id:'japan',label:'Japonya A/B',plug:'A/B',voltage:100,frequency:60,search:'Türkiye fişi Japonya Type A B seyahat adaptörü'},
    au:{id:'au',label:'Avustralya ve Yeni Zelanda I',plug:'I',voltage:230,frequency:50,search:'Türkiye fişi Type I Avustralya seyahat adaptörü topraklı'},
    ch:{id:'ch',label:'İsviçre J',plug:'J',voltage:230,frequency:50,search:'Türkiye fişi Type J İsviçre seyahat adaptörü'},
    unknown:{id:'unknown',label:'Diğer veya bilinmiyor',plug:'?',voltage:null,frequency:null,search:''}
  };
  ...
  function evaluate(raw={}){
    ...
    const frequencyCompatible=supportsFrequency(input.frequency,destination.frequency);
    ...
    if(input.frequency==='unknown'){
      return {...base,status:'evidence',title:'Frekans etiketini doğrulayın',summary:`Hedef şebeke yaklaşık ${destination.frequency} Hz. Cihazın 50/60 Hz kabulü bilinmeden kesin uygunluk verilemez.`,actions:['Cihaz veya adaptör INPUT etiketindeki Hz satırını kontrol edin.','Motor, saat, pompa ve zamanlama kullanan cihazlarda frekans farkını üreticiyle doğrulayın.']};
    }

    if(!frequencyCompatible&&frequencySensitive){
      return {...base,status:'frequency_mismatch',title:'Frekans uyumu doğrulanmadı',summary:`Cihaz etiketi ${input.frequency==='50'?'50':'60'} Hz; hedef şebeke ${destination.frequency} Hz. Fiş adaptörü frekansı değiştirmez.`,actions:['Motorlu veya zamanlamaya bağlı cihazı üretici onayı olmadan çalıştırmayın.','Hedef ülkede uygun cihaz kullanmayı veya profesyonel dönüştürme çözümünü değerlendirin.']};
    }
