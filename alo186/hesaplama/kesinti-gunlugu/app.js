(function () {
  ...
  function loadState() {
    try {
      ...
      return {
        entries: parsed.entries.map(core.normalizeEntry),
  ...
  function currentSettingsFromForm() {
    const priorities = {};
    $$('[data-priority]').forEach((input) => { priorities[input.dataset.priority] = input.checked; });
  ...
  function populateSettings() {
    $('#usage').value = state.settings.usage;
    $('#voltage').value = state.settings.voltage;
    $('#area').value = state.settings.area;
    $$('[data-priority]').forEach((input) => { input.checked = Boolean(state.settings.priorities[input.dataset.priority]); });
  }

  function populateYears() {
    const current = new Date().getFullYear();
    const years = new Set([current, ...state.entries.map((entry) => Number(entry.date.slice(0, 4)))]);
    const currentSelection = Number($('#yearFilter').value) || current;
    $('#yearFilter').innerHTML = [...years].sort((a, b) => b - a).map((year) => `<option value="${year}">${year}</option>`).join('');
    $('#yearFilter').value = years.has(currentSelection) ? String(currentSelection) : String(current);
  }
  ...
  function renderSignals(summary) {
    ...
    if (summary.damageEntries.length) {
      items.push(`<div class="signal review"><strong>${summary.damageEntries.length} cihaz hasarı şüphesi işaretlendi.</strong><span>EPDK tüketici bilgisinde dağıtım şebekesi kaynaklı hasar talebi için zararın ortaya çıktığı tarihten itibaren 10 iş günlük süre açıklanır. Yetkili servis raporu ve resmî başvuru kaydını gecikmeden hazırlayın.</span></div>`);
    }
    $('#rightsSignals').innerHTML = items.join('');
  }

  function renderRoutes(summary) {
    const decision = core.buildResilienceRoutes(summary);
    $('#routeList').innerHTML = decision.routes.map((route) => `<a class="route-card" href="${route.href}" data-route="${route.id}"><strong>${route.label} →</strong><span>${route.reason}</span></a>`).join('') || '<p class="info">En az bir kesinti kaydı ve kritik ihtiyaç seçtiğinizde ücretsiz hesaplama rotaları burada görünür.</p>';
    $('#affiliateRoute').classList.toggle('hidden', !decision.showProductCenter);
    $('#medicalBlock').classList.toggle('hidden', !decision.commercialSuppressed);
    $$('[data-route]').forEach((link) => link.addEventListener('click', () => emit('outage_resilience_route_opened', { route: link.dataset.route })));
  }
  ...
  function render() {
    ...
    $('#dashboard').classList.toggle('hidden', state.entries.length === 0);
    $('#metricCount').textContent = String(summary.count);
    $('#metricYear').textContent = `${year} takvim yılı`;
    $('#metricTotal').textContent = formatDuration(summary.totalMinutes);
    $('#metricLongest').textContent = formatDuration(summary.longestMinutes);
    $('#metricLongSignal').textContent = summary.longDurationEntries.length ? `${summary.longDurationEntries.length} kayıt 12 saati aşıyor` : '12 saat sinyali yok';
    $('#metricReview').textContent = summary.hasCompensationReviewSignal ? 'Resmî kontrol önerilir' : (summary.count ? 'Kayıtları sürdürün' : 'Kayıt bekleniyor');
    ...
    $('#emptyState').classList.toggle('hidden', summary.count > 0);
    $$('[data-delete]').forEach((button) => button.addEventListener('click', () => {
      ...
      saveState();
      emit('outage_journal_entry_deleted');
      render();
    }));

    renderSignals(summary);
    $('#evidenceList').innerHTML = core.buildEvidenceChecklist(summary).map((item) => `<li>${item}</li>`).join('');
  ...
  $('#entryForm').addEventListener('submit', (event) => {
    ...
    try {
      ...
      const entry = core.normalizeEntry({
        ...
        kind: $('#kind').value,
        scope: $('#scope').value,
        officialRecord: $('#officialRecord').checked,
        deviceDamage: $('#deviceDamage').checked
      });
      ...
      $('#officialRecord').checked = false;
      $('#deviceDamage').checked = false;
      render();
    } catch (error) {
      $('#entryError').textContent = error.message || 'Kayıt eklenemedi.';
    }
  });
  ...
  $('#clearAllBtn').addEventListener('click', () => {
    ...
    populateSettings();
    emit('outage_journal_cleared');
    render();
  });
  $('#printBtn').addEventListener('click', () => { emit('outage_evidence_checklist_printed'); window.print(); });
  $('#exportBtn').addEventListener('click', () => {
    const payload = core.createExport(state.entries, state.settings);
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `alo186-kesinti-gunlugu-${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
    URL.revokeObjectURL(link.href);
    emit('outage_journal_exported', { count: state.entries.length });
  });
  $('#productCenterLink').addEventListener('click', () => emit('outage_product_center_opened'));
