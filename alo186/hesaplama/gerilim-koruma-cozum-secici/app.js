(() => {
  ...
  function setValues(data){
    ['symptom','duration','scope','measurement','loadType','powerBand','phase','continuity','existing'].forEach(key=>{
      if(data[key]!=null)$(key).value=String(data[key]);
    });
    $('emergency').checked=Boolean(data.emergency);
    $('medical').checked=Boolean(data.medical);
  }
  ...
  function render(result){
    ...
    $('solutionMetric').textContent=result.solution.label;
    $('riskMetric').textContent=result.riskLevel;
    $('distributionMetric').textContent=result.distributionReport?'Evet':'Hayır / önce teknik kontrol';
    $('commercialMetric').textContent=result.commercialAllowed?'Düşük riskli rota':'Kapalı';
    $('resultStatus').textContent=result.professionalRequired?'Uzman / resmî kanal gerekli':'Ön değerlendirme tamamlandı';
    $('resultStatus').className=`status ${result.riskLevel==='Kritik'||result.riskLevel==='Yüksek'?'bad':result.riskLevel==='Orta'?'warn':'ok'}`;
    $('recommendationLabel').textContent=result.solution.label;
    $('recommendationNote').textContent=result.solution.note;
    $('reasonList').innerHTML=list(result.reasons);
    $('limitList').innerHTML=list(result.limits.length?result.limits:['Bu sonuç kesin ürün veya arıza teşhisi değildir.']);
    $('checkList').innerHTML=list(result.checks);
    $('nextStepText').textContent='Sonucu ürün adıyla değil, ilgili ücretsiz kontrol veya resmî kanal üzerinden doğrulayın.';
    ...
    if(result.productCategory){
      $('productCenterLink').href=`https://www.alo186.com/akilli-urun-secimi?kategori=${encodeURIComponent(result.productCategory)}`;
    }
    updateAffiliate();
    $('results').focus({preventScroll:true});
    $('results').scrollIntoView({behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'start'});
    emit('voltage_protection_selector_completed',{
      recommendation:result.recommendation,
      risk_level:result.riskLevel,
      professional_required:result.professionalRequired,
      distribution_report:result.distributionReport,
      commercial_allowed:result.commercialAllowed,
      product_category:result.productCategory||'none'
    });
  }

  document.querySelectorAll('[data-preset]').forEach(button=>button.addEventListener('click',()=>{
    setValues(presets[button.dataset.preset]);
    emit('voltage_protection_preset_selected',{preset:button.dataset.preset});
  }));
  ...
  $('resetBtn').addEventListener('click',()=>{
    $('selectorForm').reset();
    setValues(presets.transient);
    $('results').classList.add('hidden');
    $('validation').textContent='';
    emit('voltage_protection_selector_reset');
  });

  $('affiliateAck').addEventListener('change',updateAffiliate);
  $('productCenterLink').addEventListener('click',event=>{
    if($('productCenterLink').getAttribute('aria-disabled')==='true'){
      event.preventDefault();
      return;
    }
    emit('voltage_protection_product_center_opened',{category:'surge_strip'});
  });
