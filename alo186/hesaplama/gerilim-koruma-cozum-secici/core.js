(function(root,factory){
  ...
  'use strict';
  const solutions={
    emergency:{label:'Acil güvenlik ve enerjiyi güvenli biçimde ayırma',note:'Duman, yanık kokusu, ark, elektrik çarpması veya ısınan pano ürün seçimiyle çözülemez.',nextStepUrl:'https://www.alo186.com/karar-motoru/',nextStepLabel:'Acil yönlendirmeyi aç',productCategory:null},
    medical_plan:{label:'Üretici onaylı profesyonel güç kalitesi planı',note:'Tıbbi veya yaşam destek cihazında genel ürün önerisi güvenli değildir.',nextStepUrl:'https://www.alo186.com/hesaplama/yedek-guc-cozum-secici/',nextStepLabel:'Profesyonel yedek güç ön değerlendirmesini aç',productCategory:null},
    neutral_risk:{label:'Nötr / bağlantı arızası için acil teknik kontrol',note:'Bazı lambaların aşırı parlak, bazılarının sönük olması nötr veya bağlantı problemiyle ilişkili olabilir.',nextStepUrl:'https://www.alo186.com/karar-motoru/',nextStepLabel:'186 mı, elektrikçi mi kararını aç',productCategory:null},
    utility_report:{label:'Dağıtım şirketi kaydı + tesisat doğrulaması',note:'Bina veya komşularda görülen gerilim/kesinti olayı önce resmî dağıtım kanalında kayda alınmalıdır.',nextStepUrl:'https://www.alo186.com/edas-bul',nextStepLabel:'Bölgemdeki EDAŞ kanalını bul',productCategory:null},
    installation_check:{label:'Bina / daire iç tesisat kontrolü',note:'Tek oda, tek devre veya bina içi sınırlı sorunlarda ürün almadan önce bağlantı ve koruma ölçümü gerekir.',nextStepUrl:'https://www.alo186.com/karar-motoru/',nextStepLabel:'Güvenli kontrol sırasını aç',productCategory:null},
    device_service:{label:'Cihaz veya adaptör teknik servisi',note:'Sorun yalnız bir cihazda ve şebeke ölçümü normalse, koruma ürünü yerine cihaz/adaptör incelemesi öne çıkar.',nextStepUrl:'https://www.alo186.com/haberler/parafudr-gerilim-koruma-rolesi-farki',nextStepLabel:'Koruma cihazlarının görev farkını öğren',productCategory:null},
    spd_layers:{label:'Katmanlı aşırı gerilim koruması',note:'Çok kısa süreli yıldırım veya anahtarlama darbelerinde SPD/parafudr kademeleri değerlendirilir.',nextStepUrl:'https://www.alo186.com/hesaplama/parafudr-risk-testi/',nextStepLabel:'Parafudr risk testini aç',productCategory:'surge_strip'},
    ups_avr:{label:'UPS / AVR ile kesintisiz elektronik yük koruması',note:'Bilgisayar, modem ve ağ cihazı yeniden başlıyorsa geçiş süresi, W/VA ve AVR aralığı birlikte değerlendirilir.',nextStepUrl:'https://www.alo186.com/hesaplama/yedek-guc-cozum-secici/',nextStepLabel:'UPS ve yedek güç çözümünü karşılaştır',productCategory:null},
    voltage_regulator:{label:'Tek cihaz için otomatik voltaj regülasyonu ön değerlendirmesi',note:'Yetkili ölçümle doğrulanmış, tekrarlayan düşük/yüksek gerilimde yalnız monofaze fişli elektronik yük için düşünülebilir.',nextStepUrl:'https://www.alo186.com/haberler/parafudr-gerilim-koruma-rolesi-farki',nextStepLabel:'Regülatör, röle, SPD ve UPS farkını aç',productCategory:null},
    ...
    diagnose_first:{label:'Önce olayın süresini ve kapsamını ayırın',note:'Belirti tek başına doğru cihazı seçmeye yetmez; şebeke, tesisat ve cihaz kaynakları ayrılmalıdır.',nextStepUrl:'https://www.alo186.com/karar-motoru/',nextStepLabel:'Belirtiye göre güvenli yönlendirmeyi aç',productCategory:null}
  };
