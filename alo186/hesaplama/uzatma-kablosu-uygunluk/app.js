(() => {
  ...
  const valueIds=['voltage','loadPower','powerFactor','loadType','startMultiplier','usage','intendedUse','length','area','ratedCurrent','reelState','woundMaxPower','unwoundMaxPower','environment','applianceClass'];
  const checkIds=['factoryAssembled','labelVerified','damageFree','earthPresent','outdoorRated','thermalProtection','daisyChain'];
  const presets={
    office:{voltage:230,loadPower:300,powerFactor:.95,loadType:'electronic',startMultiplier:'',usage:'short',intendedUse:'portable',length:15,area:1.5,ratedCurrent:16,reelState:'none',woundMaxPower:'',unwoundMaxPower:'',environment:'indoor',applianceClass:'classI',factoryAssembled:true,labelVerified:true,damageFree:true,earthPresent:true,outdoorRated:false,thermalProtection:false,daisyChain:false},
    garden:{voltage:230,loadPower:1200,powerFactor:.8,loadType:'motor',startMultiplier:3,usage:'short',intendedUse:'portable',length:30,area:2.5,ratedCurrent:16,reelState:'unwound',woundMaxPower:1000,unwoundMaxPower:3500,environment:'outdoor',applianceClass:'classI',factoryAssembled:true,labelVerified:true,damageFree:true,earthPresent:true,outdoorRated:true,thermalProtection:true,daisyChain:false},
    risky:{voltage:230,loadPower:2000,powerFactor:1,loadType:'resistive',startMultiplier:'',usage:'continuous',intendedUse:'heater',length:25,area:1.5,ratedCurrent:10,reelState:'wound',woundMaxPower:800,unwoundMaxPower:3000,environment:'indoor',applianceClass:'classI',factoryAssembled:true,labelVerified:true,damageFree:true,earthPresent:true,outdoorRated:false,thermalProtection:true,daisyChain:false}
  };
  ...
  function render(result){
    ...
    const labels={compatible:'Düşük riskli teknik sınırlar içinde',conditional:'Koşullu / etiket doğrulaması gerekli',incompatible:'Uygun değil veya riskli'};
    ...
    emit('extension_suitability_completed',{status:result.status,professional_required:result.professionalRequired,commercial_allowed:result.commercialAllowed,drop_band:result.dropPercent<=3?'low':result.dropPercent<=5?'medium':'high'});
  }
  ...
  $('resetBtn').addEventListener('click',()=>{$('extensionForm').reset();setValues(presets.office);$('results').classList.add('hidden');$('validation').textContent='';emit('extension_suitability_reset');});
  $('affiliateAck').addEventListener('change',updateAffiliate);
  $('productCenterLink').addEventListener('click',event=>{if(event.currentTarget.getAttribute('aria-disabled')==='true'){event.preventDefault();return;}emit('extension_product_center_opened',{category:'extension_cord'});});
