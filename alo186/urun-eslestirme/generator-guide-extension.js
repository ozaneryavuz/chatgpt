  ...
  function extendCatalog(){
    ...
    catalog.knowledgeGraph=options=>{
      ...
      const nodes=purchaseCollections.map(collection=>{
        ...
        return {'@type':'ItemList','@id':`https://www.alo186.com/akilli-urun-secimi#collection-${collection.id}`,name:collection.name,description:collection.description,numberOfItems:products.length,itemListElement:products.map((product,index)=>({'@type':'ListItem',position:index+1,item:{'@id':catalog.productId(product)}}))};
      });
