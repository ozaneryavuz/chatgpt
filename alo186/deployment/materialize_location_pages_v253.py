...
MARKER = 'data-alo186-location-page-v253="true"'
...
def normalize_slug(value: str) -> str:
    ...
    slug = re.sub(r"[^a-z0-9]+", "-", value.translate(table).casefold()).strip("-")
    if not slug:
        raise ValueError("Boş il/şirket slugı üretilemez")
    return slug


def load_catalog(repo_root: Path) -> tuple[dict[int, str], list[Company]]:
    path = Path(repo_root) / CATALOG
    if not path.is_file():
        raise FileNotFoundError(f"Türkiye EDAŞ kataloğu bulunamadı: {path}")
    source = path.read_text(encoding="utf-8", errors="strict")
    province_block = re.search(r"const\s+provinceNames\s*=\s*\{(.*?)\};", source, re.DOTALL)
...
def company_phrase(companies: list[Company]) -> str:
...
def city_page(city: str, companies: list[Company]) -> str:
    slug = normalize_slug(city)
    canonical = f"{ORIGIN}/il/{slug}"
    providers = company_phrase(companies)
    if len(companies) == 2 and any(company.district_mode for company in companies):
        direct = (
            f"{city} için yetkili dağıtım şirketi bulunduğunuz yakaya göre {providers}. "
            "Şebeke kesintisi veya dağıtım arızası için 186 aranır; ilçe ve yaka seçimi resmî kesinti ekranı açılmadan önce doğrulanır."
        )
        detail = (
        ...
        direct = (
            f"{city} için yetkili dağıtım şirketi {providers}. Şebeke kesintisi veya dağıtım arızası "
            "için 186 aranır; planlı kesinti bilgisi yetkili şirketin resmî kanalından doğrulanır."
        )
        detail = (
            f"{city} elektrik dağıtım hizmetinden {providers} sorumludur. ALO186 kesinti süresi tahmini "
...
<section class="answer" aria-labelledby="dogrudan-yanit"><h2 id="dogrudan-yanit">{html.escape(city)} elektrik kesintisi için nere aranır?</h2><p><strong>{html.escape(direct)}</strong></p></section>
<div class="actions"><a class="button" href="tel:186">186’yı ara</a><a class="button secondary" href="/edas-bul?q={query}">Resmî kesinti kanalını bul</a></div>
...
<p class="fine">Şirketin telefon, WhatsApp, kesinti ekranı ve hizmet bağlantıları değişebilir. İşlem öncesinde resmî şirket ekranındaki güncel bilgiyi yeniden doğrulayın.</p>'''
    return shell(
        ...
        description=(
            f"{city} elektrik kesintisinde 186, 112 ve yetkili dağıtım şirketi ayrımını görün; "
            "resmî kesinti kanalına kişisel veri paylaşmadan ilerleyin."
        ),
        canonical=canonical,
        body=body,
    )
...
def company_page(company: Company, provinces: dict[int, str]) -> str:
    ...
    area_links = "".join(
        f'<li><a href="/il/{normalize_slug(city)}">{html.escape(city)} elektrik kesintisi ve 186 rehberi</a></li>'
        for city in areas
    )
    special = ""
...
<section class="answer"><h2>{html.escape(company.name)} elektrik kesintisi için nere aranır?</h2><p><strong>{html.escape(company.name)} hizmet bölgesindeki şebeke kesintisi ve dağıtım arızaları için 186 aranır. Planlı kesinti, sayaç ve şebeke işlemleri şirketin resmî kanalından doğrulanır.</strong></p></section>
<div class="actions"><a class="button" href="tel:186">186’yı ara</a><a class="button secondary" href="/edas-bul?q={query}">Resmî kesinti kanalını bul</a></div>
...
<p class="fine">ALO186, {html.escape(company.name)} adına kayıt veya ihbar almaz. Güncel telefon, kesinti ekranı ve işlem koşulları şirketin resmî kanalında doğrulanmalıdır.</p>'''
    return shell(
        ...
        description=(
            f"{company.name} için 186 arıza hattı, 112 güvenlik ayrımı, hizmet verdiği iller "
            "ve resmî kesinti kanalına yönlendirme."
        ),
        canonical=canonical,
        body=body,
    )
...
def write_page(path: Path, content: str) -> str:
...
def update_sitemap(site: Path, urls: list[str]) -> dict[str, int]:
    ...
    existing = set(re.findall(r"<loc>(.*?)</loc>", source, re.DOTALL))
    ...
    return {"expected": len(urls), "existing": len(urls) - len(missing), "added": len(missing)}
...
def materialize(repo_root: Path, site: Path) -> dict[str, object]:
    ...
    for province_id, city in sorted(provinces.items()):
        ...
        state = write_page(target, city_page(city, companies_by_province[province_id]))
        city_states[state] += 1
        urls.append(f"{ORIGIN}/il/{slug}")

    for company in companies:
        target = site / COMPANY_ROOT / company.slug / "index.html"
        state = write_page(target, company_page(company, provinces))
        company_states[state] += 1
        urls.append(f"{ORIGIN}/dagitim-sirketleri/{company.slug}")

    sitemap = update_sitemap(site, urls)
    return {
        "version": VERSION,
