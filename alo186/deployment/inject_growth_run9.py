...
def update_release(site: Path, base_path: str, cards: int, offline: list[str]) -> None:
    ...
    if pages_path.is_file():
        ...
        pages["growthRun9"] = {**contract, "basePath": base_path, "routes": [public_url(base_path, route) for route in ROUTES.values()], "entryCardsInjected": cards, "offlineAdded": offline}
...
def run(site: Path, base_path: str) -> dict:
    ...
    recompute(site)
    return {"ok": True, "basePath": base_path, "routes": [public_url(base_path, route) for route in ROUTES.values()], "entryCardsInjected": cards, "offlineAdded": offline, "rawPersonalDataCollected": False, "directAffiliateLinksAdded": 0, "noBuyOutcomePreserved": True}
