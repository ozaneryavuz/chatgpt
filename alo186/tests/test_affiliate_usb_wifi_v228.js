...
const ROUTE = path.join(ROOT, 'amazon-elektrik-urunleri', 'bilgisayar-usb-wifi-adaptor-secimi');
const HTML = fs.readFileSync(path.join(ROUTE, 'index.html'), 'utf8');
const APP = fs.readFileSync(path.join(ROUTE, 'app-v228.js'), 'utf8');
const catalog = require(path.join(ROUTE, 'catalog-v228.js'));
const EXPECTED = new Map([
  ['B0859M539M', 'Archer T3U Plus'],
  ['B07P5PRK7J', 'Archer T2U Plus'],
  ['B09KTDXPY3', 'Archer T3U Nano'],
]);
...
assert.strictEqual(catalog.version, 228);
...
assert.strictEqual(catalog.category.requiredTool, 'embedded-usb-wifi-compatibility-v228');
assert.ok(catalog.category.excludes.includes('enterprise-managed-network'));
assert.ok(catalog.category.excludes.includes('critical-infrastructure-network'));
...
const canonical = 'https://alo186.com/amazon-elektrik-urunleri/bilgisayar-usb-wifi-adaptor-secimi/';
...
console.log(JSON.stringify({
  ok: true,
  route: '/amazon-elektrik-urunleri/bilgisayar-usb-wifi-adaptor-secimi/',
