...
SLUGS = (
    'bess-insulation-monitoring-device-imd-izolasyon-hatasi-teshis',
    'frekans-inverteri-sto-active-safe-torque-off-motor-calismiyor',
    'ups-epo-active-emergency-power-off-uzak-acil-kapatma-teshis',
)
OVERLAY = ROOT / "alo186/deployment/routing-overlays/content-authority-run110.json"
...
def test_authority_content_run110() -> None:
    ...
    bess = (ROOT / f"alo186/haberler/{SLUGS[0]}/index.html").read_text(encoding="utf-8")
    sto = (ROOT / f"alo186/haberler/{SLUGS[1]}/index.html").read_text(encoding="utf-8")
    epo = (ROOT / f"alo186/haberler/{SLUGS[2]}/index.html").read_text(encoding="utf-8")
    for phrase in ("IMD", "izolasyon direnci", "IT sistemi", "rack", "PCS", "IEC 61557-8"):
        assert phrase.casefold() in bess.casefold()
    for phrase in ("STO Active", "Safe Torque Off", "çift kanal", "safety rölesi", "24 V", "IEC 61800-5-2"):
        assert phrase.casefold() in sto.casefold()
    for phrase in ("EPO Active", "Emergency Power Off", "uzak kontak", "static bypass", "akü kesicisi", "IEC 62040-1"):
        assert phrase.casefold() in epo.casefold()
    print("ALO186 içerik otoritesi run110: PASS")
