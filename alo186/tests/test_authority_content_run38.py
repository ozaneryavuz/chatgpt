...
ARTICLES = [
    {
        "slug": "elektrik-sayaci-isiniyor-yanik-kokusu-cizirti-edas-elektrikci",
        "required": [
            "Sayaç yanma / hasar", "186", "mührü açmayın", "15 iş günü",
        ...
        "source_hosts": ["epdk.gov.tr", "enerjisa.com.tr", "gedizperakende.com.tr"],
    },
    {
        ...
        "links": [
            ...
            "/haberler/harmonik-k-factor-trafo-derating-notr-isinmasi",
            "/haberler/kompanzasyon-detuned-reaktor-yuzdesi-rezonans-frekansi",
            "/hesaplama/elektrik-kanit-envanteri/",
            "/hesaplama/teknik-devir-kabul-paketi/",
            "/hizmetler/elektrik-surekliligi-izleme/",
        ],
        "source_hosts": ["ieee.org", "se.com", "schneider-electric.com", "fluke.com"],
    },
    {
        "slug": "ups-notr-toprak-bypass-4-kutup-ats-npe",
        "required": [
            "N–PE", "statik bypass", "bakım bypassı", "4 kutuplu ATS",
            "solid neutral", "separately derived", "backfeed", "Satın almama sınırı",
            "Son doğrulama: 29 Temmuz 2026",
        ],
        "links": [
...
def main() -> None:
    ...
    assert manifest["version"] >= 68, manifest["version"]
    article_routes = [route for route in manifest["routes"] if route["type"] == "article"]
    assert len(article_routes) >= 153, len(article_routes)
