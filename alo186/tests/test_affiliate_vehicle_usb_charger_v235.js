...
const ROUTE=path.join(ROOT,'amazon-elektrik-urunleri','arac-prizi-usb-sarj-gucu-olcum-secimi');
const HTML=fs.readFileSync(path.join(ROUTE,'index.html'),'utf8');
const APP=fs.readFileSync(path.join(ROUTE,'app-v235.js'),'utf8');
const catalog=require(path.join(ROUTE,'catalog-v235.js'));
const EXPECTED=new Map([['B0DWT464C3','ACP08700'],['B0DFWZSW3R','ACP08447'],['B08PRZ815M','ACP02562']]);
...
assert.strictEqual(catalog.version,235);
...
assert.strictEqual(catalog.category.requiredTool,'embedded-vehicle-accessory-socket-usb-power-measurement-v235');
...
const canonical='https://alo186.com/amazon-elektrik-urunleri/arac-prizi-usb-sarj-gucu-olcum-secimi/';
...
console.log(JSON.stringify({ok:true,route:'/amazon-elektrik-urunleri/arac-prizi-usb-sarj-gucu-olcum-secimi/',products:[...EXPECTED.keys()],knowledgeGraph:['Product','Brand','ItemList','identifier','additionalProperty'],affiliatePolicy:'after_tool',professionalOnlyBypass:false,staleBoundary:'45-open-46-closed',duplicateAsin:false}));
