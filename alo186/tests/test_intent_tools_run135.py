...
def source_contracts() -> None:
    ...
    for path, html in zip(PAGES, texts, strict=True):
        assert "<!doctype html>" in html.casefold(), path
        for token in (
            'name="description"',
            'rel="canonical"',
            '"@type":"WebApplication"',
    ...
    actual = {item["canonicalPath"] for item in overlay["routes"]}
    assert actual == set(ROUTES), (actual, ROUTES)
...
def artifact_contracts(site: Path, base_path: str) -> None:
    ...
    for route in ROUTES:
        assert route in sitemap_paths, (route, sorted(path for path in sitemap_paths if "hesaplama" in path)[-20:])
    ...
    for route in ROUTES:
        assert route in active, (route, sorted(path for path in active if "kesinti" in path or "sarj" in path))
    ...
    for route in ROUTES:
        ...
        assert f'href="{expected}"' in hub, (route, expected)
    match = re.search(r"(\d+)\s+çekirdek araç", hub)
    assert match and int(match.group(1)) >= 52, match.group(0) if match else "missing"
