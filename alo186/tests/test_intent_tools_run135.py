...
def source_contracts() -> None:
    ...
    for path, html in zip(PAGES, texts, strict=True):
        for token in ('name="description"','rel="canonical"','"@type":"WebApplication"','"@type":"FAQPage"','"@type":"BreadcrumbList"','Doğrudan cevap','Son kaynak doğrulama: 2 Ağustos 2026'):
            assert token in html, (path, token)
        for forbidden in ('"@type":"Product"','"@type":"Offer"','"@type":"AggregateRating"','amazon.com.tr','alo186rehber-21','priceCurrency','availability'):
            assert forbidden not in html, (path, forbidden)

    compensation, pv_backup, ev_rcd = texts
    for token in ('12 saati','30 gün','Hizmet Kalitesi Yönetmeliğinin 26 ncı maddesine göre','hours>8760','aria-invalid','/edas-bul'):
        assert token in compensation, token
    assert 'on iş günü' not in compensation.casefold()

    for token in ('anti-islanding',"pv==='unknown'",'Önce inverter modelini ve topolojiyi doğrulayın','hours<0.5||hours>72','Hibrit etiketi yeterli değildir','geri besleme yapmayın'):
        assert token in pv_backup, token

    for token in ('IEC 62752','IEC 62955','Mode 2: IC-CPD + üst tesisat Tip A adayı','Mode 3: Tip A + doğrulanmış 6 mA DC RDC-DD adayı',"mode==='unknown'","external==='none'",'yalnız standart Tip AC','ayrı devre'):
    ...
    for token in ('data-alo186-intent-tools-run135="true"','TARGET = Path("hesaplama/index.html")','pages-release.json','intentToolsRun135','directMarketplaceLinks','recompute_checksums'):
...
def artifact_contracts(site: Path, base_path: str) -> None:
    ...
    for route in ROUTES:
        assert route in sitemap_paths, route
    ...
    assert contract["routes"] == [f"{base_path}{route}" if base_path else route for route in ROUTES]
