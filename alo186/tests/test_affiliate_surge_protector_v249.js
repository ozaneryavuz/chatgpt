...
const slug = "akim-korumali-priz-yuk-uygunluk-secimi";
...
const catalogText = fs.readFileSync(path.join(route, "catalog-v249.js"), "utf8");
const appText = fs.readFileSync(path.join(route, "app-v249.js"), "utf8");
const overlay = JSON.parse(fs.readFileSync(path.join(ROOT, "alo186", "deployment", "routing-overlays", "249-affiliate-surge-protector.json"), "utf8"));

const sandbox = { module: { exports: {} }, exports: {}, globalThis: {} };
vm.runInNewContext(catalogText, sandbox, { filename: "catalog-v249.js" });
const catalog = sandbox.module.exports;

assert.strictEqual(catalog.version, 249);
...
const expectedAsins = ["B07CST4766", "B08KW6X13Y", "B09YTYTZ4J"];
const expectedMpns = ["TSK6136", "TSK5015", "CT-9186"];
...
for (const product of catalog.products) {
  ...
  assert.match(product.technicalSource, /^https:\/\/(tuncmatik\.com|cata\.com\.tr)\//);
...
const canonical = "https://alo186.com/amazon-elektrik-urunleri/akim-korumali-priz-yuk-uygunluk-secimi/";
...
assert.ok(html.includes("Mevcut sağlam") || html.includes("Mevcut korumalı"));
...
for (const gate of ["gateConsumerUse", "gateGrounded", "gateLoad", "gateNoHighRisk", "gateCondition", "gateProtectionLimit", "gateExisting", "gateVariant", "gateNeed", "gateAffiliate"]) {
...
assert.strictEqual(overlay.version, 249);
assert.strictEqual(overlay.routes.length, 1);
assert.strictEqual(overlay.routes[0].canonicalPath, "/amazon-elektrik-urunleri/akim-korumali-priz-yuk-uygunluk-secimi/");
assert.strictEqual(overlay.routes[0].source, "alo186/amazon-elektrik-urunleri/akim-korumali-priz-yuk-uygunluk-secimi/index.html");
