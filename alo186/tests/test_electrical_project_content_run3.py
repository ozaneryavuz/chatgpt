...
ROUTES = {
    "/haberler/62-villa-22-kw-ac-2x180-kw-dc-elektrikli-arac-sarj-altyapisi": ROOT / "alo186/haberler/62-villa-22-kw-ac-2x180-kw-dc-elektrikli-arac-sarj-altyapisi/index.html",
    "/haberler/kisa-devre-secicilik-gerilim-dusumu-kablo-koruma-koordinasyon-raporu": ROOT / "alo186/haberler/kisa-devre-secicilik-gerilim-dusumu-kablo-koruma-koordinasyon-raporu/index.html",
    "/haberler/elektrik-odalari-saftlar-kablo-tava-busbar-yangin-durdurucu-detaylari": ROOT / "alo186/haberler/elektrik-odalari-saftlar-kablo-tava-busbar-yangin-durdurucu-detaylari/index.html",
}
OVERLAY = ROOT / "alo186/deployment/routing-overlays/electrical-project-content-run3-v296.json"
REQUIRED_HEADINGS = (
...
def main() -> None:
    ...
    assert overlay["generatedAt"] == "2026-08-05"
    ...
    for route, path in ROUTES.items():
        ...
        assert "Son doğrulama: 5 Ağustos 2026" in html
        assert "ALO186; EDAŞ, TEDAŞ, EPDK, EMO, GİB veya başka bir kamu kuruluşu değildir" in html
        assert "proje onayı" in folded or "proje veya koruma ayarı onayı" in folded
        ...
        assert article["dateModified"] == "2026-08-05"
    ...
    for index, (left_route, left_words) in enumerate(pairs):
        for right_route, right_words in pairs[index + 1 :]:
            ...
            assert similarity < 0.35, (left_route, right_route, similarity)
