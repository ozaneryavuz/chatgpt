...
PAGES = {
    "rcd": ROOT / "haberler/kacak-akim-rolesi-tum-fisler-cekiliyken-atiyor-ortak-notr-n-pe-teshis/index.html",
    "ev": ROOT / "haberler/ev-sarj-11-kw-yerine-7-kw-yavas-sarj-faz-akim-limiti-teshis/index.html",
    "ges": ROOT / "haberler/ges-uretim-var-fatura-yuksek-oz-tuketim-sayac-mahsuplasma-kontrolu/index.html",
}
OVERLAY = ROOT / "deployment/routing-overlays/content-authority-run130.json"
...
def test_topic_specific_fail_closed_guards() -> None:
    rcd = PAGES["rcd"].read_text(encoding="utf-8")
    assert all(term in rcd for term in ["RCD’yi köprülemeyin", "ortak nötr", "N-PE"])
    ev = PAGES["ev"].read_text(encoding="utf-8")
    assert all(term in ev for term in ["onboard charger", "akım limitini", "L1/L2/L3"])
    ges = PAGES["ges"].read_text(encoding="utf-8")
    assert all(term in ges for term in ["Portal ekran görüntüsünü", "öz tüketim", "import/export"])


def test_routing_overlay() -> None:
    data = json.loads(OVERLAY.read_text(encoding="utf-8"))
    assert data["version"] == 208
    assert data["generatedAt"] == "2026-08-02"
    assert len(data["routes"]) == 3
    for route in data["routes"]:
        assert route["path"].startswith("/haberler/")
        assert (ROOT / route["file"]).exists()
    assert data["qualityGate"].endswith("test_authority_content_run130.py")
...
if __name__ == "__main__":
    ...
    print("content authority run130: PASS")
