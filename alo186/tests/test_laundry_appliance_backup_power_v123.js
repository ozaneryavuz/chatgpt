...
for(const basePath of ['','/chatgpt']){
  ...
  const routeDir=path.join(target,'hesaplama',route);
  const page=fs.readFileSync(path.join(routeDir,'index.html'),'utf8');
  const runtime=fs.readFileSync(path.join(routeDir,'app.js'),'utf8');
