...
const ROUTE = path.join(ROOT, 'amazon-elektrik-urunleri', 'usb-c-35mm-ses-adaptoru-secimi');
const HTML = fs.readFileSync(path.join(ROUTE, 'index.html'), 'utf8');
const APP = fs.readFileSync(path.join(ROUTE, 'app-v224.js'), 'utf8');
const catalog = require(path.join(ROUTE, 'catalog-v224.js'));
const EXPECTED = new Map([
  ['B0D7MN4W7H', 'MW2Q3ZM/A'],
  ['B082WG5VTK', '80154'],
  ['B01LYUKQ0L', 'CATL54-01'],
]);
...
assert.strictEqual(catalog.version, 224);
...
assert.strictEqual(catalog.category.requiredTool, 'embedded-usb-c-audio-compatibility-v224');
...
assert.strictEqual(catalog.verificationStatus(new Date('2026-09-16T12:00:00Z')).fresh, true, '45th day must remain fresh');
assert.strictEqual(catalog.verificationStatus(new Date('2026-09-17T12:00:00Z')).fresh, false, '46th day must fail closed');

const canonical = 'https://alo186.com/amazon-elektrik-urunleri/usb-c-35mm-ses-adaptoru-secimi/';
...
console.log(JSON.stringify({
  ok: true,
  route: '/amazon-elektrik-urunleri/usb-c-35mm-ses-adaptoru-secimi/',
