...
const productNodes=(graph['@graph']||[]).filter((node)=>node&&node['@type']==='Product');
const ugreenNode=productNodes.find((node)=>node.sku==='ugreen-nexode-140w-90322');
...
assert.equal(audit.productCount,productNodes.length,'Canonical audit gerçek Product düğümü sayısını vermeli.');
assert.ok(audit.productCount>0,'Affiliate grafiğinde doğrulanmış Product düğümü bulunmalı.');
