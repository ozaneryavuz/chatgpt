...
assert.ok(catalog.products.length>=11);
...
for(const id of ['philips-spn7040wa-62','tuncmatik-tsk6134','brennenstuhl-eco-line-6','baseus-crystal-shine-100w-2m'])assert.ok(ids.has(id),`Yeni ürün eksik: ${id}`);
...
assert.deepEqual([...new Set(publicProducts.map(product=>product.category))].sort(),['powerbank','usb_c_cable']);
...
const baseusNode=productNodes.find(node=>node.sku==='baseus-crystal-shine-100w-2m');
assert.ok(baseusNode);
assert.equal(baseusNode.mpn,'CAJY000701');
assert.ok(baseusNode.additionalProperty.some(item=>item.name==='maxPowerW'&&item.value===100));
assert.ok(baseusNode.additionalProperty.some(item=>item.name==='cableM'&&item.value===2));
