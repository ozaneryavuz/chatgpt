...
assert.strictEqual(result.matches.length,1,'20.000 mAh / 65 W filtresi yalnız doğrulanmış güçlü ürünü döndürmeli.');
...
assert(result.matches.length>=2,'Kablosuz 10.000 mAh ihtiyacı en az iki seçenek üretmeli.');
...
assert.strictEqual(result.matches.length,1,'5 priz / 900 J filtresi yalnız teknik minimumu doğrulanan ürünü döndürmeli.');
...
assert.strictEqual(result.matches.length,1,'USB gereksinimi yalnız USB ürününü döndürmeli.');
assert.strictEqual(result.matches[0].product.asin,'B08KW6X13Y');
assert(result.matches[0].unknowns.length>0,'Bilinmeyen teknik alan görünür olmalı.');
...
console.log('Ürün kataloğu ve eşleştirme testleri başarılı.');