...
ALO = ROOT / "alo186"
ROUTING = ALO / "deployment/routing-overlays/content-authority-v377-ground-rcd-frequency.json"

ARTICLES = {
    "topraklama-direnci-dusmuyor-ilave-kazik-neden-ise-yaramiyor": {
        "canonical": "https://alo186.com/haberler/topraklama-direnci-dusmuyor-ilave-kazik-neden-ise-yaramiyor",
        "sources": ("fluke.com", "webstore.iec.ch"),
        "needles": ("Wenner", "fall-of-potential", "özdirenci", "elektrot"),
    },
    "30ma-300ma-kacak-akim-rolesi-ayni-anda-atiyor-selektivite": {
        "canonical": "https://alo186.com/haberler/30ma-300ma-kacak-akim-rolesi-ayni-anda-atiyor-selektivite",
        "sources": ("webstore.iec.ch", "se.com"),
        "needles": ("S tipi", "30 mA", "300 mA", "selektivite"),
    },
    "ges-inverter-grid-frequency-high-low-sebeke-frekansi-alarmi": {
        "canonical": "https://alo186.com/haberler/ges-inverter-grid-frequency-high-low-sebeke-frekansi-alarmi",
        "sources": ("manuals.sma.de", "support.huawei.com", "teias.gov.tr"),
        "needles": ("Grid Frequency", "Hz", "ülke/grid profili", "şebeke operatörü"),
    },
}
...
def intent_tokens(text: str) -> set[str]:
    ...
    stop = {
        ...
        "kac", "olmali", "dogru", "secimi", "ayni", "anda", "atiyor",
    }
...
class ContentAuthorityV377Tests(unittest.TestCase):
    ...
    def test_professional_only_and_no_buy_first_contract(self) -> None:
        for slug in ARTICLES:
            ...
            self.assertRegex(html, r"satın almayın|satın almak yerine|değiştirmeyin|büyütmeyin", slug)

    def test_routing_overlay_is_exact_and_no_buy_first(self) -> None:
        data = json.loads(ROUTING.read_text(encoding="utf-8"))
        self.assertEqual(377, data["version"])
