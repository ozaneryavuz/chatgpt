...
OVERLAY = ROOT / "alo186/deployment/routing-overlays/electrical-project-content-run4-v314.json"
REQUIRED_HEADINGS = (
    ...
    "Doğrulanan resmî ve standart kaynaklar",
    "İlgili ALO186 bağlantıları",
...
def main() -> None:
    ...
    for route, path in ROUTES.items():
        ...
        assert "Son doğrulama: 6 Ağustos 2026" in html
        assert "ALO186; EDAŞ, TEDAŞ, EPDK, EMO, GİB" in html
        assert "kamu kuruluşu değildir" in html
        assert "proje onayı" in folded or "proje veya kabul onayı" in folded
        assert "mevzuata tam uyum" in folded
        assert all(token not in folded for token in FORBIDDEN_COMMERCIAL)
        ...
        assert len(article.get("citation", [])) >= 4
