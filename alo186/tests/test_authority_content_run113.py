...
SLUGS = (
    'ev-sarj-dinamik-yuk-yonetimi-sayac-ct-yonu-modbus-teshis',
    'ges-inverter-grid-overvoltage-ac-gerilim-yukselmesi-kablo-empedans-teshis',
    'jenerator-ats-3-kutup-4-kutup-anahtarlanan-notr-rcd-teshis',
)
OVERLAY = ROOT / "alo186/deployment/routing-overlays/content-authority-run113.json"
...
def test_authority_content_run113() -> None:
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    assert overlay["version"] == 186
    ...
    ats = (ROOT / f"alo186/haberler/{SLUGS[2]}/index.html").read_text(encoding="utf-8")
    for phrase in ("Dynamic Load Management", "CT yönü", "Modbus", "fail-safe", "Power Sharing", "ana kesici"):
        assert phrase.casefold() in ev.casefold()
    for phrase in ("Grid Overvoltage", "voltage rise", "PCC", "AC kablo", "on dakikalık", "Volt-Var"):
        assert phrase.casefold() in pv.casefold()
    for phrase in ("Solid Neutral", "Switched Neutral", "4 kutup", "N-PE", "RCD", "Separately Derived"):
        assert phrase.casefold() in ats.casefold()
    print("ALO186 içerik otoritesi run113: PASS")
