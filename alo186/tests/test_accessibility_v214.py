...
ATTR_PATTERN = re.compile(r"([\w:-]+)(?:\s*=\s*([\"\'])(.*?)\2|\s*=\s*([^\s>]+))?", re.DOTALL)
...
def assert_css_contract(css: str) -> None:
    ...
    assert "overflow-x:hidden" not in compact.replace(" ", "").lower()
    assert "overflow-inline:hidden" not in compact.replace(" ", "").lower()
...
def assert_bundle(site: Path, release: dict) -> None:
    ...
    assert count_media_fallback_candidates(site) >= 1
...
def test_source_contract() -> None:
    ...
    article = (ROOT / "alo186/haberler/alo186-article.css").read_text(encoding="utf-8")
    for token in (
        ".top nav a,.toc a,.sources a,.footer a",
        "min-height:44px",
        ".article figure>img:not([width])",
        "aspect-ratio:16/9",
...
def test_canonical_and_pages_modes() -> None:
    with tempfile.TemporaryDirectory() as directory:
        base = Path(directory)
        canonical = base / "canonical"
        release = build(ROOT, canonical, "accessibility-v214-test")
        assert_bundle(canonical, release)
...
def main() -> None:
    ...
    test_canonical_and_pages_modes()
    print(json.dumps({"ok": True, "version": 214, "minimumTouchTargetPx": 44}, ensure_ascii=False))
