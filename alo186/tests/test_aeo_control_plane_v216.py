...
def main() -> None:
    ...
    assert len(json.loads((root/"alo186/aeo/intent-registry-v216.json").read_text())["intents"])>=15
    assert len(json.loads((root/"alo186/aeo/ai-citation-benchmark-v216.json").read_text())["queries"])>=15
