...
ARTICLES = [
    {
        "slug": "ups-jenerator-boyutlandirma-thdi-guc-faktoru",
        "required": [
            "maksimum giriş akımı", "akü şarj", "giriş güç faktörü", "THDi", "ramp-in",
            "Evrensel 1,5 kat kuralı yoktur", "jeneratör geçici", "Son doğrulama: 29 Temmuz 2026",
        ],
        "links": [
            "/haberler/ups-bypass-senkron-degil-kaynak-tolerans-disi",
        ...
        "source_hosts": ["eaton.com", "se.com", "productinfo.se.com"],
    },
    {
        "slug": "parafudr-yedek-sigorta-sccr-kisa-devre-koordinasyonu",
        "required": [
            "maksimum yedek sigorta", "I<sub>SCCR</sub>", "beklenen kısa devre akımı",
            "branş bağlantı", "V tipi geçişli bağlantı", "iç termik ayırıcı",
            "tek bir evrensel amper değeri yoktur", "Son doğrulama: 29 Temmuz 2026",
        ],
        "links": [
        ...
        "source_hosts": ["iec.ch", "dehn-international.com", "phoenixcontact.com"],
    },
    {
        "slug": "vpp-temel-tuketim-baseline-performans-dogrulama",
        "required": [
            "temel tüketim değeri", "baseline", "gerçekleşen tüketim", "kontrol sayacı",
            "60 saniyelik", "24 saate kadar", "5 yıl", "4 gün",
        ...
        "source_hosts": ["teias.gov.tr", "epdk.gov.tr"],
    },
]
...
def main() -> None:
    ...
    assert manifest["version"] >= 57, manifest["version"]
    article_routes = [route for route in manifest["routes"] if route["type"] == "article"]
    assert len(article_routes) >= 126, len(article_routes)
    ...
    ups = (REPO_ROOT / "alo186/haberler/ups-jenerator-boyutlandirma-thdi-guc-faktoru/index.html").read_text(encoding="utf-8").casefold()
    spd = (REPO_ROOT / "alo186/haberler/parafudr-yedek-sigorta-sccr-kisa-devre-koordinasyonu/index.html").read_text(encoding="utf-8").casefold()
    vpp = (REPO_ROOT / "alo186/haberler/vpp-temel-tuketim-baseline-performans-dogrulama/index.html").read_text(encoding="utf-8").casefold()
    assert "yetersiz jeneratör" in ups and "aküde kalabileceğini" in ups
    assert "aynı ürünün veri sayfasında" in spd and "başka ürüne kopyalanamaz" in spd
    assert "aylık faturadaki toplam kwh" in vpp and "resmî metodoloji" in vpp
    assert "gelir tahmini, garanti veya resmî performans onayı vermez" in vpp
