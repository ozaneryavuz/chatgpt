...
ROUTING = ROOT / "deployment/routing-overlays/growth-rcd-grounding-v365.json"
...
def test_routing_overlay_v365_owns_only_new_routes():
    text = read(ROUTING)
    assert '"version": 365' in text
    for route in (
        "/haberler/kacak-akim-rolesi-test-dugmesi-ne-ise-yarar/",
        "/hesaplama/kacak-akim-test-gunlugu/",
    ):
...
if __name__ == "__main__":
    ...
    test_routing_overlay_v365_owns_only_new_routes()
    print({"ok": True, "version": 365, "merchantLinks": 0, "newAffiliateClasses": 0})
