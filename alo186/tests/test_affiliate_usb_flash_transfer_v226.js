...
const ROUTE = path.join(ROOT, 'amazon-elektrik-urunleri', 'usb-bellek-dosya-aktarim-secimi');
const HTML = fs.readFileSync(path.join(ROUTE, 'index.html'), 'utf8');
const APP = fs.readFileSync(path.join(ROUTE, 'app-v226.js'), 'utf8');
const catalog = require(path.join(ROUTE, 'catalog-v226.js'));
const EXPECTED = new Map([
  ['B07YYK13LF', 'SDDDC3-064G-G46'],
  ['B0B1JNDZ5J', 'DTXM/64GB'],
  ['B015CH1NAQ', 'SDCZ73-064G-G46'],
]);
...
assert.strictEqual(catalog.version, 226);
...
assert.strictEqual(catalog.category.requiredTool, 'embedded-usb-flash-transfer-fit-v226');
...
for (const product of catalog.products) {
  ...
  for (const field of ['userNeed', 'strengths', 'limitations', 'noBuyWhen', 'technicalSource', 'amazonVerification', 'verifiedAt', 'attributes']) {
...
const canonical = 'https://alo186.com/amazon-elektrik-urunleri/usb-bellek-dosya-aktarim-secimi/';
...
assert.ok(APP.includes("risk === 'consumer-low'"));
...
for (const product of products) {
  ...
  assert.ok(Array.isArray(product.additionalProperty) && product.additionalProperty.length >= 4);
...
console.log(JSON.stringify({
  ok: true,
  route: '/amazon-elektrik-urunleri/usb-bellek-dosya-aktarim-secimi/',
