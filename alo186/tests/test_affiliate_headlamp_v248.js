...
const slug = "elektrik-kesintisi-kafa-feneri-eller-serbest-aydinlatma-secimi";
...
const catalogText = fs.readFileSync(path.join(route, "catalog-v248.js"), "utf8");
const appText = fs.readFileSync(path.join(route, "app-v248.js"), "utf8");
const overlay = JSON.parse(fs.readFileSync(path.join(ROOT, "alo186", "deployment", "routing-overlays", "248-affiliate-headlamp.json"), "utf8"));

const sandbox = { module: { exports: {} }, exports: {}, globalThis: {} };
vm.runInNewContext(catalogText, sandbox, { filename: "catalog-v248.js" });
const catalog = sandbox.module.exports;

assert.strictEqual(catalog.version, 248);
...
const expectedAsins = ["B0CJNXYPRL", "B0BMQDDQLL", "B0CF9RYKY6"];
const expectedMpns = ["CT-9120", "CT-9123", "502803"];
...
for (const product of catalog.products) {
  ...
  assert.match(product.technicalSource, /^https:\/\/(www\.cimenelektrik\.com|www\.buyukalpler\.com\.tr|www\.ledlenser\.com\.tr)\//);
...
const canonical = "https://alo186.com/amazon-elektrik-urunleri/elektrik-kesintisi-kafa-feneri-eller-serbest-aydinlatma-secimi/";
...
assert.ok(html.includes("Mevcut fener") || html.includes("Mevcut sağlam kafa lambanız"));
assert.strictEqual((html.match(/rel="sponsored nofollow noopener"/g) || []).length, 3);
assert.ok(!/href="https:\/\/www\.amazon\.com\.tr\/dp\/[^\"]+\?tag=/.test(html), "affiliate links initially locked");
...
for (const gate of ["gateOutageUse", "gateDeenergized", "gateNoEmergency", "gateEnvironment", "gateBattery", "gateLightNeed", "gateRuntime", "gateExisting", "gateVariant", "gateNeed", "gateAffiliate"]) {
...
assert.strictEqual(overlay.version, 248);
assert.strictEqual(overlay.routes.length, 1);
assert.strictEqual(overlay.routes[0].canonicalPath, "/amazon-elektrik-urunleri/elektrik-kesintisi-kafa-feneri-eller-serbest-aydinlatma-secimi/");
assert.strictEqual(overlay.routes[0].source, "alo186/amazon-elektrik-urunleri/elektrik-kesintisi-kafa-feneri-eller-serbest-aydinlatma-secimi/index.html");
