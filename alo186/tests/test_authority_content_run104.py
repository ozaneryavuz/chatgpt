...
SLUGS = ('frekans-inverteri-earth-fault-ground-fault-motor-kablo-izolasyon-teshis', 'ev-sarj-dinamik-yuk-dengeleme-ct-faz-modbus-akim-siniri-teshis', 'vpp-toplayici-komut-teslimat-uzlastirma-sayac-telemetri-soc')
OVERLAY = ROOT / "alo186/deployment/routing-overlays/content-authority-run104.json"
...
def test_authority_content_run104() -> None:
    ...
    vfd = (ROOT / f"alo186/haberler/{SLUGS[0]}/index.html").read_text(encoding="utf-8")
    ...
    for phrase in ("Earth Fault", "motor kablosu", "izolasyon", "PE", "PWM", "IEC 61800-5-1"):
        assert phrase.casefold() in vfd.casefold()
    for phrase in ("dinamik yük dengeleme", "CT yönü", "faz eşleşmesi", "Modbus", "OCPP", "fallback"):
        assert phrase.casefold() in ev.casefold()
    for phrase in ("toplayıcı", "uzlaştırma", "telemetri", "SoC", "komut", "sayaç"):
        assert phrase.casefold() in vpp.casefold()
    print("ALO186 içerik otoritesi run104: PASS")
