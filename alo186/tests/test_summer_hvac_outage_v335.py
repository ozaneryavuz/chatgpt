...
SITE = ROOT / "alo186"
OVERLAY = SITE / "deployment/routing-overlays/summer-hvac-outage-v335.json"
GUIDE = SITE / "haberler/elektrik-kesintisi-sonrasi-klima-neden-calismiyor/index.html"
PLANNER = SITE / "hesaplama/klima-elektrik-kesintisi-yeniden-baslatma-plani/index.html"
PRO = SITE / "sektor-rehberi/otel-vrf-klima-elektrik-kesintisi-surekliligi/index.html"
ROUTES = {
    "/haberler/elektrik-kesintisi-sonrasi-klima-neden-calismiyor/": GUIDE,
    "/hesaplama/klima-elektrik-kesintisi-yeniden-baslatma-plani/": PLANNER,
    "/sektor-rehberi/otel-vrf-klima-elektrik-kesintisi-surekliligi/": PRO,
}
...
def main() -> None:
    ...
    guide_visible = text(guide).casefold()
    for required in (
        "auto-restart",
        "tekrar tekrar resetleme",
        "koruma gecikmesi",
        "doğaçlama yedek güç",
        "doğrudan amazon ürünü önerilmez",
    ):
    ...
    for required in (
        ...
        "professional-only",
        "/sektor-rehberi/otel-vrf-klima-elektrik-kesintisi-surekliligi/",
    ):
