...
SLUGS = (
    'jenerator-low-oil-pressure-dusuk-yag-basinci-sensor-gercek-basinci-teshis',
    'ev-sarj-dinamik-yuk-yonetimi-anti-tripping-ct-sayac-teshis',
    'ges-pid-potential-induced-degradation-anti-pid-teshis',
)
OVERLAY = ROOT / "alo186/deployment/routing-overlays/content-authority-run115.json"
...
def test_authority_content_run115() -> None:
    ...
    ev = (ROOT / f"alo186/haberler/{SLUGS[1]}/index.html").read_text(encoding="utf-8")
    pid = (ROOT / f"alo186/haberler/{SLUGS[2]}/index.html").read_text(encoding="utf-8")
    for phrase in ("Low Oil Pressure", "mekanik manometre", "Oil Pressure Sender", "crank disconnect", "yağ pompası", "shutdown"):
        assert phrase.casefold() in generator.casefold()
    for phrase in ("Dynamic Load Management", "Anti-Tripping", "CT", "site limiti", "minimum şarj akımı", "fail-safe"):
        assert phrase.casefold() in ev.casefold()
    for phrase in ("Potential Induced Degradation", "IEC TS 62804-1", "I-V", "EL", "anti-PID", "system voltage"):
        assert phrase.casefold() in pid.casefold()
    print("ALO186 içerik otoritesi run115: PASS")
