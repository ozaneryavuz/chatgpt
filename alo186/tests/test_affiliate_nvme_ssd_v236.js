...
const ROUTE=path.join(ROOT,'amazon-elektrik-urunleri','bilgisayar-m2-nvme-ssd-uyumluluk-secimi');
const HTML=fs.readFileSync(path.join(ROUTE,'index.html'),'utf8');
const APP=fs.readFileSync(path.join(ROUTE,'app-v236.js'),'utf8');
const catalog=require(path.join(ROUTE,'catalog-v236.js'));
const EXPECTED=new Map([['B0DBR3DZWG','SNV3S/1000G'],['B0CN5L6FPK','LSD10Z001TG8'],['B08THW4S3T','MZ-V8V500BW']]);
...
assert.strictEqual(catalog.version,236);
...
assert.strictEqual(catalog.category.requiredTool,'embedded-nvme-slot-backup-compatibility-v236');
...
for(const product of catalog.products){
  ...
  assert.ok(/^https:\/\/(?:www\.)?(?:kingston\.com|europe\.kioxia\.com|semiconductor\.samsung\.com)\//.test(product.technicalSource));
...
const canonical='https://alo186.com/amazon-elektrik-urunleri/bilgisayar-m2-nvme-ssd-uyumluluk-secimi/';
...
assert.ok(!/href="https:\/\/www\.amazon\.com\.tr\/(?:dp|s\?k=)/i.test(HTML),'Initial HTML must not contain enabled Amazon href');
...
for(const product of products){
  ...
  assert.ok(product.identifier.some((value)=>value.propertyID==='ASIN'&&EXPECTED.has(value.value)));
  assert.ok(product.identifier.some((value)=>value.propertyID==='MPN'));
...
console.log(JSON.stringify({ok:true,route:'/amazon-elektrik-urunleri/bilgisayar-m2-nvme-ssd-uyumluluk-secimi/',products:[...EXPECTED.keys()],knowledgeGraph:['Product','Brand','ItemList','identifier','additionalProperty'],affiliatePolicy:'after_tool',professionalOnlyBypass:false,staleBoundary:'45-open-46-closed',duplicateAsin:false}));
