...
SITE = ROOT / "alo186"
OVERLAY = SITE / "deployment/routing-overlays/computer-nas-outage-journey-v326.json"
GUIDE = SITE / "haberler/elektrik-kesintisi-bilgisayara-zarar-verir-mi/index.html"
PLANNER = SITE / "hesaplama/bilgisayar-nas-elektrik-kesintisi-hazirlik-plani/index.html"
SELECTOR = SITE / "amazon-elektrik-urunleri/bilgisayar-nas-ups-secimi/index.html"
ROUTES = {
    "/haberler/elektrik-kesintisi-bilgisayara-zarar-verir-mi/": GUIDE,
    "/hesaplama/bilgisayar-nas-elektrik-kesintisi-hazirlik-plani/": PLANNER,
    "/amazon-elektrik-urunleri/bilgisayar-nas-ups-secimi/": SELECTOR,
}
...
def main() -> None:
    ...
    guide_visible = text(GUIDE.read_text(encoding="utf-8"))
    for required in (
        "Aktif elektrik veya yangın tehlikesinde test ve ticaret kapalıdır",
        "Laptop",
        "NAS / küçük sunucu",
        "USB veya ağ üzerinden güvenli kapanma desteği",
    ):
        assert required in guide_visible
    ...
    for required in (
        ...
        "/amazon-elektrik-urunleri/bilgisayar-nas-ups-secimi/",
        "/hesaplama/modem-ont-elektrik-kesintisi-internet-hazirlik-plani/",
    ):
        assert required in planner or required in planner_visible

    selector = SELECTOR.read_text(encoding="utf-8")
    selector_visible = text(selector)
    ...
    assert "USB haberleşmeli bilgisayar / NAS UPS sınıfı" in selector_visible
    assert "Priz tipi enerji ölçer / watt metre sınıfı" in selector_visible
    assert "Bağlantılar kilitli" in selector_visible
    assert "Professional-only veya affiliate dışı sınıflar" in selector_visible
