...
PAGES = {
    "generator": ROOT / "haberler/jenerator-underfrequency-dusuk-frekans-governor-yuk-adimi-teshis/index.html",
    "pv": ROOT / "haberler/ges-inverter-reverse-polarity-ters-kutup-string-polarite-teshis/index.html",
    "ups": ROOT / "haberler/ups-fan-failure-overtemperature-hava-akisi-filtre-teshis/index.html",
}
OVERLAY = ROOT / "deployment/routing-overlays/content-authority-run127.json"
...
def test_topic_specific_fail_closed_guards() -> None:
    ...
    assert all(term in generator for term in ["Underfrequency korumasını köprülemeyin", "bağımsız frekans", "yük adımı"])
    pv = PAGES["pv"].read_text(encoding="utf-8")
    assert all(term in pv for term in ["PV konnektörünü yük altında ayırmayın", "Ters polariteyi yalnız yetkin personel", "string–MPPT"])
    ups = PAGES["ups"].read_text(encoding="utf-8")
    assert all(term in ups for term in ["Fan alarmını köprülemeyin", "UPS kapağını enerjiliyken açmayın", "bakım bypassına plansız geçmeyin"])


def test_routing_overlay() -> None:
    data = json.loads(OVERLAY.read_text(encoding="utf-8"))
    assert data["version"] == 205
    assert len(data["routes"]) == 3
    for route in data["routes"]:
        assert route["path"].startswith("/haberler/")
        assert (ROOT / route["file"]).exists()
    assert data["qualityGate"].endswith("test_authority_content_run127.py")
...
if __name__ == "__main__":
    ...
    print("content authority run127: PASS")
