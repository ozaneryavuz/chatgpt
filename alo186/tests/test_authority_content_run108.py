...
SLUGS = (
    'elektrik-dusuk-yuksek-gerilim-teknik-kalite-olcumu-edas-basvuru',
    'jenerator-battery-charger-fail-dusuk-mars-akusu-gerilimi-teshis',
    'ges-inverter-dc-injection-dcinj-fault-ac-akim-dc-bilesen-teshis',
)
OVERLAY = ROOT / "alo186/deployment/routing-overlays/content-authority-run108.json"
...
def test_authority_content_run108() -> None:
    ...
    edas = (ROOT / f"alo186/haberler/{SLUGS[0]}/index.html").read_text(encoding="utf-8")
    genset = (ROOT / f"alo186/haberler/{SLUGS[1]}/index.html").read_text(encoding="utf-8")
    pv = (ROOT / f"alo186/haberler/{SLUGS[2]}/index.html").read_text(encoding="utf-8")
    for phrase in ("teknik kalite", "bir haftalık ölçüm", "flicker", "IEC 61000-4-30", "Class A", "EDAŞ"):
        assert phrase.casefold() in edas.casefold()
    for phrase in ("Battery Charger Fail", "Charge Fail", "marş aküsü", "float", "bekleme", "ISO 8528-12"):
        assert phrase.casefold() in genset.casefold()
    for phrase in ("DCInj-FAULT", "DC injection", "AC çıkış akımı", "ILeak", "grid rölesi", "IEC 61727"):
        assert phrase.casefold() in pv.casefold()
    print("ALO186 içerik otoritesi run108: PASS")
