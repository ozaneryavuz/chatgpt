...
const ROUTE=path.join(ROOT,'amazon-elektrik-urunleri','laptop-usb-ethernet-adaptoru-hiz-uygunluk-secimi');
const HTML=fs.readFileSync(path.join(ROUTE,'index.html'),'utf8');
const APP=fs.readFileSync(path.join(ROUTE,'app-v237.js'),'utf8');
const catalog=require(path.join(ROUTE,'catalog-v237.js'));
const EXPECTED=new Map([['B08HQBC678','UE300C'],['B09GRL3VCN','UE306'],['B0DSCDGD4G','UE302C']]);
...
assert.strictEqual(catalog.category.requiredTool,'embedded-usb-ethernet-link-measurement-v237');
...
for(const product of catalog.products){
  ...
  for(const field of ['userNeed','strengths','limitations','noBuyWhen','technicalSource','amazonStatusSource','verifiedAt']) assert.ok(product[field],`${product.asin} missing ${field}`);
  ...
  assert.ok(product.amazonStatusSource.startsWith('https://www.epey.com/'));
...
const canonical='https://alo186.com/amazon-elektrik-urunleri/laptop-usb-ethernet-adaptoru-hiz-uygunluk-secimi/';
...
assert.ok(!APP.includes('localStorage')); assert.ok(!APP.includes('fetch('));
...
const products=graph.filter((node)=>node['@type']==='Product'); const lists=graph.filter((node)=>node['@type']==='ItemList');
...
console.log(JSON.stringify({ok:true,route:'/amazon-elektrik-urunleri/laptop-usb-ethernet-adaptoru-hiz-uygunluk-secimi/',products:[...EXPECTED.keys()],knowledgeGraph:['Product','Brand','ItemList','identifier','additionalProperty'],affiliatePolicy:'after_tool',professionalOnlyBypass:false,staleBoundary:'45-open-46-closed',duplicateAsin:false,revisitReason:'private-90-day-ics'}));
