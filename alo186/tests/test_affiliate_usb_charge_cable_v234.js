...
const ROUTE=path.join(ROOT,'amazon-elektrik-urunleri','usb-sarj-kablosu-guc-protokol-olcum-secimi');
...
const EXPECTED=new Map([['B0B46PHW14','CAJY000701'],['B0F2MH3GFW','ACA08717'],['B0F2M377PZ','ACA08715']]);
...
assert.strictEqual(catalog.category.requiredTool,'embedded-usb-charge-cable-power-protocol-measurement-v234');
...
const canonical='https://alo186.com/amazon-elektrik-urunleri/usb-sarj-kablosu-guc-protokol-olcum-secimi/';
...
console.log(JSON.stringify({ok:true,route:'/amazon-elektrik-urunleri/usb-sarj-kablosu-guc-protokol-olcum-secimi/',products:[...EXPECTED.keys()],knowledgeGraph:['Product','Brand','ItemList','identifier','additionalProperty'],affiliatePolicy:'after_tool',professionalOnlyBypass:false,staleBoundary:'45-open-46-closed',duplicateAsin:false}));
