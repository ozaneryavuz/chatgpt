...
const slug = "arac-usb-sarj-adaptoru-guc-uyumluluk-secimi";
...
const catalogText = fs.readFileSync(path.join(route, "catalog-v247.js"), "utf8");
const appText = fs.readFileSync(path.join(route, "app-v247.js"), "utf8");
const overlay = JSON.parse(fs.readFileSync(path.join(ROOT, "alo186", "deployment", "routing-overlays", "247-affiliate-car-usb-charger.json"), "utf8"));

const sandbox = { module: { exports: {} }, exports: {}, globalThis: {} };
vm.runInNewContext(catalogText, sandbox, { filename: "catalog-v247.js" });
const catalog = sandbox.module.exports;

assert.strictEqual(catalog.version, 247);
...
const expectedAsins = ["B08558MGST", "B0BTP9GF27", "B0BT4GWMS3"];
const expectedMpns = ["CCB001btBK", "CCA004btBK", "BXAC65C"];
...
for (const product of catalog.products) {
  ...
  assert.match(product.technicalSource, /^https:\/\/(www\.belkin\.com|bix\.com\.tr)\//);
...
const canonical = "https://alo186.com/amazon-elektrik-urunleri/arac-usb-sarj-adaptoru-guc-uyumluluk-secimi/";
...
assert.ok(html.includes("Mevcut adaptör"));
...
for (const gate of ["gateVehicleSocket", "gateVoltage", "gateNoDamage", "gateDeviceProtocol", "gateCable", "gatePower", "gateNoHighRisk", "gateExisting", "gateVariant", "gateNeed", "gateAffiliate"]) {
...
assert.strictEqual(overlay.version, 247);
assert.strictEqual(overlay.routes.length, 1);
assert.strictEqual(overlay.routes[0].canonicalPath, "/amazon-elektrik-urunleri/arac-usb-sarj-adaptoru-guc-uyumluluk-secimi/");
assert.strictEqual(overlay.routes[0].source, "alo186/amazon-elektrik-urunleri/arac-usb-sarj-adaptoru-guc-uyumluluk-secimi/index.html");
