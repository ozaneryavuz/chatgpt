(() => {
  ...
  function categoryState(category, now) {
    const all = catalog.products.filter((product) => product.category === category.id && catalog.isCatalogProduct(product));
    const fresh = all.filter((product) => catalog.verificationStatus(product, now).fresh);
    const stale = all.length - fresh.length;
    const manufacturerSearch = fresh.filter((product) => product.status === 'manufacturer_verified_search').length;
    if (category.affiliatePolicy === 'professional_only') {
      return { key: 'professional', label: 'Affiliate kapalı', detail: 'Ürün yerine güvenli yönlendirme ve profesyonel ölçüm gerekir.', all, fresh, stale, manufacturerSearch };
    }
    if (category.affiliatePolicy === 'after_tool') {
      const graphDetail = manufacturerSearch ? ` ${manufacturerSearch} üretici kaynaklı tam model düğümü bilgi grafiğinde görülebilir.` : '';
      return { key: 'tool', label: 'Önce ücretsiz araç', detail: `Yük, süre, gerilim, güç veya uyumluluk hesabı tamamlanmadan ürün rotası açılmaz.${graphDetail}`, all, fresh, stale, manufacturerSearch };
    }
    if (category.affiliatePolicy === 'after_checklist') {
      return { key: 'checklist', label: 'Önce kontrol listesi', detail: 'Standart, ürün ömrü ve kullanım koşulu doğrulandıktan sonra kategori karşılaştırması yapılır.', all, fresh, stale, manufacturerSearch };
    }
    if (fresh.length) {
      return { key: 'fresh', label: 'Güncel doğrulanmış kart', detail: `${fresh.length} kart 45 günlük teknik doğrulama sınırı içinde.`, all, fresh, stale, manufacturerSearch };
    }
    if (stale) {
      return { key: 'stale', label: 'Doğrulama yenileme bekliyor', detail: `${stale} kartın doğrulama süresi geçtiği için doğrudan ürün bağlantısı kapalı.`, all, fresh, stale, manufacturerSearch };
    }
    return { key: 'empty', label: 'Doğrulanmış kart yok', detail: 'ALO186 sırf sonuç üretmek için eksik veya kaynaksız ürün önermez.', all, fresh, stale, manufacturerSearch };
  }

  function categoryAction(category, state) {
    if (state.manufacturerSearch) return { href: `/urun-bilgi-grafigi/?kategori=${encodeURIComponent(category.id)}`, label: 'Ürün düğümlerini ve kaynakları gör' };
  ...
  document.addEventListener('DOMContentLoaded', () => {
    if (!catalog || !Array.isArray(catalog.categories) || !Array.isArray(catalog.products) || typeof catalog.isCatalogProduct !== 'function') {
      byId('categoryGrid').innerHTML = '<p class="error">Katalog verisi yüklenemedi. Ürün bağlantısı gösterilmedi.</p>';
      return;
    }

    const now = new Date();
    const verified = catalog.products.filter((product) => catalog.isCatalogProduct(product));
    ...
    byId('categoryGrid').innerHTML = catalog.categories.map((category) => {
      ...
      return `<article class="category-card state-${state.key}"><div class="card-head"><span class="status">${escapeHtml(state.label)}</span><h3>${escapeHtml(category.name)}</h3></div><p>${escapeHtml(category.description)}</p><dl><div><dt>Affiliate politikası</dt><dd>${escapeHtml(category.affiliatePolicy)}</dd></div><div><dt>Doğrulanmış düğüm</dt><dd>${state.all.length}</dd></div><div><dt>Güncel / eski</dt><dd>${state.fresh.length} / ${state.stale}</dd></div><div><dt>Üretici kaynaklı arama</dt><dd>${state.manufacturerSearch}</dd></div></dl><p class="state-detail">${escapeHtml(state.detail)}</p><a class="button secondary category-action" data-category="${escapeHtml(category.id)}" data-policy="${escapeHtml(category.affiliatePolicy)}" data-status="${escapeHtml(state.key)}" href="${escapeHtml(action.href)}">${escapeHtml(action.label)}</a></article>`;
    }).join('');
