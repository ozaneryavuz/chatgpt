(() => {
  'use strict';
  const start = () => {
    const catalog = window.Alo186UsbCPdPpsChargerCatalogV233;
    const toolChecks = ['toolNeed','toolWatt','toolProtocol','toolCable','toolOutlet','toolScope','toolExisting'];
    ...
    const sync = () => {
      ...
      const categoryAllowed = catalog.category.affiliatePolicy === 'after_tool'
        && catalog.category.requiredTool === 'embedded-usbc-pd-pps-charger-measurement-v233'
      ...
      if (!freshness.fresh) gateStatus.textContent = 'Teknik ve Amazon Türkiye doğrulaması 45 günü aştı; bağlantılar kapalı.';
      else if (!toolPassed) gateStatus.textContent = 'Önce watt, protokol, kablo, priz ve kullanım güvenliği kontrollerini tamamlayın.';
      ...
      else gateStatus.textContent = 'Koşullar tamamlandı. Amazon kaydında ASIN, MPN, port sayısı ve güç dağılımını yeniden doğrulayın.';
    };
    toolButton.addEventListener('click', () => {
      toolPassed = checked(toolChecks);
      toolResult.dataset.passed = String(toolPassed);
      toolResult.textContent = toolPassed
        ? 'Ön kontrol geçti: yalnız sağlam prizde, kritik olmayan tüketici cihazları için watt ve protokol uyumu karşılaştırılabilir.'
        : 'Ön kontrol geçmedi: watt, PD/PPS protokolü, kablo, priz, mevcut çözüm ve kritik olmayan kullanım birlikte doğrulanmalıdır.';
      sync();
    });
