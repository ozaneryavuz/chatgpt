(function () {
  "use strict";
  const catalog = window.ALO186IndoorThermoHygrometerCatalogV245;
  if (!catalog) return;
  const readinessIds = ["gateIndoor", "gateDry", "gatePlacement", "gateRange", "gateReference", "gateNotCritical", "gateVariant"];
  ...
  function update() {
    if (!readinessIds.every(checked)) {
      return lock("Önce iç ortam, kuruluk, konum, ölçüm aralığı, referans kontrolü, kritik olmayan kullanım ve doğru varyant kontrollerini tamamlayın.");
    }
    ...
    status.textContent = opened
      ? "Kontroller tamamlandı. Amazon Türkiye sayfasında ASIN, model, renk ve paket varyantını yeniden doğrulayın."
  ...
  lock("Mağaza bağlantıları güvenlik, uygunluk ve ihtiyaç kontrolleri tamamlanana kadar kapalıdır.");
})();
