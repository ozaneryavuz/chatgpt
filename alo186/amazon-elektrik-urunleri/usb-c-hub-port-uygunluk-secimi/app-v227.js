(() => {
  ...
  const start = () => {
    const catalog = window.Alo186UsbCHubCatalogV227;
    const toolChecks = ['toolHost', 'toolPorts', 'toolVideo', 'toolPower', 'toolExisting'];
    ...
    const sync = () => {
      ...
      const categoryAllowed = catalog.category.affiliatePolicy === 'after_tool'
        && catalog.category.requiredTool === 'embedded-usb-c-hub-port-fit-v227'
      ...
      if (!freshness.fresh) {
        ...
        gateStatus.textContent = 'Mağaza bağlantıları kapalı. Önce ana cihaz, port, görüntü, güç ve mevcut çözüm koşullarını doğrulayın.';
        ...
        gateStatus.textContent = 'Koşullar tamamlandı. ASIN, üretici kodu, tam cihaz uyumu ve gerekli portları Amazon ile üretici kaynağında yeniden doğrulayın.';
      }
    };
    ...
    toolButton.addEventListener('click', () => {
      ...
      toolResult.textContent = toolPassed
        ? 'Ön kontrol geçti: yalnız kişisel tüketici cihazında, doğrulanan port ve güç ihtiyacına göre hub modelleri karşılaştırılabilir.'
        : 'Ön kontrol geçmedi: ana USB-C portunun işlevleri, gerekli portlar, görüntü standardı, PD zinciri ve mevcut çözüm birlikte doğrulanmalıdır.';
      sync();
    });
