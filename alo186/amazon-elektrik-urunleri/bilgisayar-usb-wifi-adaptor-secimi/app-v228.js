(() => {
  ...
  const start = () => {
    const catalog = window.Alo186UsbWifiCatalogV228;
    const toolChecks = ['toolUsb', 'toolOs', 'toolRouter', 'toolRootCause', 'toolScope'];
    ...
    const sync = () => {
      ...
      const categoryAllowed = catalog.category.affiliatePolicy === 'after_tool'
        && catalog.category.requiredTool === 'embedded-usb-wifi-compatibility-v228'
      ...
      if (!freshness.fresh) {
        ...
        gateStatus.textContent = 'Mağaza bağlantıları kapalı. Önce USB portunu, işletim sistemi sürücüsünü, router bantlarını, kök nedeni ve kullanım kapsamını doğrulayın.';
        ...
        gateStatus.textContent = 'Koşullar tamamlandı. ASIN, model, donanım sürümü ve işletim sistemi desteğini Amazon ile üretici kaynağında yeniden doğrulayın.';
      }
    };
    ...
    toolButton.addEventListener('click', () => {
      ...
      toolResult.textContent = toolPassed
        ? 'Ön kontrol geçti: yalnız kişisel tüketici bilgisayarında USB Wi-Fi adaptörleri karşılaştırılabilir.'
        : 'Ön kontrol geçmedi: USB portu, işletim sistemi desteği, router bandı, bağlantı sorununun kök nedeni ve profesyonel olmayan kullanım kapsamı birlikte doğrulanmalıdır.';
      sync();
    });
