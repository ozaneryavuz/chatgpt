(function () {
  "use strict";
  const catalog = window.ALO186SmartPlugCatalogV250;
  if (!catalog) return;
  ...
  function update() {
    if (!readinessIds.every(checked)) {
      return lock("Önce kullanım türü, topraklama, yük, yüksek risk dışlama, fiziksel durum, ortam, gerekli özellik ve ürün varyantını doğrulayın.");
    }
    ...
    if (status) {
      status.textContent = opened
        ? "Kontroller tamamlandı. Amazon Türkiye sayfasında ASIN, MPN, akım, güç, priz tipi ve ölçüm işlevini yeniden doğrulayın."
