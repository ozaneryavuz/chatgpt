(function () {
  ...
  function refresh() {
    ...
    lock();
    if (!open) {
      gateStatus.textContent = fresh ? 'Ön kontrol ve satış ortaklığı onayı tamamlanmadan Amazon bağlantıları açılmaz.' : 'Ürün doğrulama süresi doldu. Amazon bağlantıları yenileme yapılana kadar kapalıdır.';
      return;
    }
    document.querySelectorAll('.shop').forEach((link) => {
  ...
  evaluate.addEventListener('click', () => {
    ...
    result.textContent = toolPassed ? 'Ön kontrol geçti. İhtiyaca uygun ürünleri sınırlarıyla karşılaştırabilirsiniz.' : 'Kontrollerin tamamı geçmedi. Yeni soğutucu satın almak yerine bakım, güvenlik veya hava girişi sorununu çözün.';
    refresh();
  });
