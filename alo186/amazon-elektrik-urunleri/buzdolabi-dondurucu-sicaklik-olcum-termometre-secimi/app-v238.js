(() => {
  'use strict';
  const start = () => {
    const catalog = window.Alo186ColdTemperatureCatalogV238;
    const toolChecks = ['toolNeed','toolPlacement','toolUse','toolFunction','toolExisting','toolIdentity'];
    ...
    const sync = () => {
      ...
      const categoryAllowed = catalog.category.affiliatePolicy === 'after_tool'
        && catalog.category.requiredTool === 'embedded-cold-temperature-measurement-safety-v238'
      ...
      else if (!toolPassed) gateStatus.textContent = 'Önce ölçüm ihtiyacı, yerleşim, kullanım kapsamı ve gerekli işlevi doğrulayın.';
      ...
      else gateStatus.textContent = 'Koşullar tamamlandı. Amazon kaydında ASIN, MPN ve ölçüm işlevini yeniden doğrulayın.';
    };
    ...
    toolButton.addEventListener('click', () => {
      ...
      toolResult.textContent = toolPassed
        ? 'Ön kontrol geçti: ev tipi buzdolabı veya dondurucu sıcaklık ölçümü için ürünler karşılaştırılabilir.'
        : 'Ön kontrol geçmedi: gerçek ihtiyaç, doğru yerleşim, tüketici kullanımı, gerekli işlev, mevcut çözüm ve ürün kimliği birlikte doğrulanmalıdır.';
      sync();
    });
