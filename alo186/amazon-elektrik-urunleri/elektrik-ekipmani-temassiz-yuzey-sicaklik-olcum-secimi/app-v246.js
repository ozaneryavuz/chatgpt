(function () {
  "use strict";
  const catalog = window.ALO186IRSurfaceTemperatureCatalogV246;
  if (!catalog) return;
  ...
  function update() {
    if (!readinessIds.every(checked)) {
      return lock("Önce dış yüzey, kapalı muhafaza, acil durum, enerjili erişim, ortam, emisivite, hedef boyutu, karar sınırı, mevcut araç ve ürün kimliği kontrollerini tamamlayın.");
    }
    ...
    status.textContent = opened
      ? "Kontroller tamamlandı. Amazon Türkiye sayfasında ASIN, MPN, ölçüm aralığı ve optik oranı yeniden doğrulayın."
