(() => {
  'use strict';
  const start = () => {
    const catalog = window.Alo186RfRemoteSocketCatalogV237;
    const toolChecks = ['toolNeed','toolLoad','toolNoRiskLoad','toolEnvironment','toolExisting','toolIdentity'];
    ...
    const sync = () => {
      ...
      const categoryAllowed = catalog.category.affiliatePolicy === 'after_tool'
        && catalog.category.requiredTool === 'embedded-rf-remote-socket-load-safety-v237'
      ...
      else if (!toolPassed) gateStatus.textContent = 'Önce yük gücü, kullanım ortamı ve satın almama koşullarını doğrulayın.';
      ...
      else gateStatus.textContent = 'Koşullar tamamlandı. Amazon kaydında ASIN, MPN, priz tipi ve güç sınırını yeniden doğrulayın.';
    };
    toolButton.addEventListener('click', () => {
      toolPassed = checked(toolChecks);
      toolResult.dataset.passed = String(toolPassed);
      toolResult.textContent = toolPassed
        ? 'Ön kontrol geçti: yalnız kritik olmayan ve ürün güç sınırının altında kalan uygun ortam yükleri karşılaştırılabilir.'
        : 'Ön kontrol geçmedi: gerçek ihtiyaç, yük gücü, riskli yük dışlama, ortam, mevcut çözüm ve ürün kimliği birlikte doğrulanmalıdır.';
      sync();
    });
