(() => {
  ...
  const start = () => {
    const catalog = window.Alo186ModemWifiRangeCatalogV231;
    const toolChecks = ['toolNearRouter', 'toolDeadZone', 'toolMidpoint', 'toolPlacement', 'toolScope'];
    ...
    const sync = () => {
      ...
      const categoryAllowed = catalog.category.affiliatePolicy === 'after_tool'
        && catalog.category.requiredTool === 'embedded-modem-wifi-range-measurement-v231'
      ...
      if (!freshness.fresh) {
        ...
        gateStatus.textContent = 'Mağaza bağlantıları kapalı. Önce modem yanı bağlantı, ölü nokta, ara nokta sinyali, yerleşim ve kullanım kapsamını doğrulayın.';
      } else if (!checked(commerceChecks)) {
        gateStatus.textContent = 'Kapsama ön kontrolü geçti. Gerçek ihtiyaç ve satış ortaklığı açıklamasını ayrıca doğrulayın.';
      } else if (!categoryAllowed) {
        gateStatus.textContent = 'Kategori güven sözleşmesi doğrulanamadı. Mağaza bağlantıları kapalı.';
      } else {
        gateStatus.textContent = 'Koşullar tamamlandı. ASIN, MPN, donanım sürümü, yönlendirici uyumu ve bağlantı noktası hızını yeniden doğrulayın.';
      }
    };
    ...
    toolButton.addEventListener('click', () => {
      ...
      toolResult.textContent = toolPassed
        ? 'Ön kontrol geçti: yalnız ölçülmüş ev veya ev-ofis Wi‑Fi kapsama açığı için düşük riskli menzil genişleticiler karşılaştırılabilir.'
        : 'Ön kontrol geçmedi: internet hattı, modem yanı bağlantı, ara nokta sinyali, modem yerleşimi ve kritik olmayan kullanım kapsamı birlikte doğrulanmalıdır.';
      sync();
    });
