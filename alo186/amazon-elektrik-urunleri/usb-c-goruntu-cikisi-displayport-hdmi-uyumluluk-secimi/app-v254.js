(function () {
  "use strict";
  const catalog = window.ALO186UsbCDisplayCatalogV254;
  if (!catalog) return;
  ...
  function update() {
    if (!readinessIds.every(checked)) {
      return lock("Önce port durumu, DP Alt Mode, bağlantı yönü, ekran girişi, hedef çözünürlük, kullanım riski ve ürün varyantını doğrulayın.");
    }
    ...
    if (status) {
      status.textContent = opened
        ? "Kontroller tamamlandı. Amazon Türkiye sayfasında ASIN, MPN, yön, bağlantı türü ve teknik sınıfı yeniden doğrulayın."
  ...
  lock("Mağaza bağlantıları uyumluluk ve ihtiyaç kontrolleri tamamlanana kadar kapalıdır.");
})();
