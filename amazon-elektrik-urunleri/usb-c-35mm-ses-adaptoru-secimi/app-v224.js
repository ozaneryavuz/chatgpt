(() => {
  ...
  const start = () => {
    const catalog = window.Alo186UsbCAudioCatalogV224;
    const toolChecks = ['toolUsbC', 'toolDigitalAudio', 'toolHeadset', 'toolExisting', 'toolScope'];
    ...
    const sync = () => {
      ...
      const categoryAllowed = catalog.category.affiliatePolicy === 'after_tool'
        && catalog.category.requiredTool === 'embedded-usb-c-audio-compatibility-v224'
      ...
      if (!freshness.fresh) {
        ...
        gateStatus.textContent = 'Mağaza bağlantıları kapalı. Önce USB-C dijital ses, kulaklık görevi, mevcut çözüm ve kullanım kapsamını doğrulayın.';
        ...
        gateStatus.textContent = 'Koşullar tamamlandı. ASIN, üretici kodu, tam cihaz uyumu ve mikrofon desteğini Amazon ile üretici kaynağında yeniden doğrulayın.';
      }
    };
    ...
    toolButton.addEventListener('click', () => {
      ...
      toolResult.textContent = toolPassed
        ? 'Ön kontrol geçti: yalnız kişisel tüketici cihazında USB-C - 3,5 mm ses adaptörleri karşılaştırılabilir.'
        : 'Ön kontrol geçmedi: USB-C dijital ses desteği, kulaklık/mikrofon görevi, mevcut çözüm ve profesyonel olmayan kullanım kapsamı birlikte doğrulanmalıdır.';
      sync();
    });
